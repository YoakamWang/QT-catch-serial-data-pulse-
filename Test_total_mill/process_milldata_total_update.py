...
class Process_Data:
    ...
    def get_data(self, file):
        ...
        # print(data)
        try:
            inner = data.inner[self.firstIndex + self.steps] - data.inner[self.firstIndex]  # 14400
            moto = data.moto[self.firstIndex + self.steps] - data.moto[self.firstIndex]
            outer = data.outer[self.firstIndex + self.steps] - data.outer[self.firstIndex]
        except Exception:
            # print(data)
            inner = data.inner[len(data.inner) - 2] - data.inner[self.firstIndex]  # 14400
            moto = data.moto[len(data.inner) - 2] - data.moto[self.firstIndex]
            outer = data.outer[len(data.inner) - 2] - data.outer[self.firstIndex]
        # print(inner)
